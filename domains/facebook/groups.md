...
garden — public, scrapable by anything. Ordinary HTTP clients or downstream
extractors work well here. Typed extraction is useful here because the sources
are heterogeneous.
...
# Hand off to a downstream extractor in the calling conversation with whatever schema
# matches the task, such as product/listing name, price, location, year, and key features.
# For example, send external_urls with a prompt and schema tailored to the source type.
```
