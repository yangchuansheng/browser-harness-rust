...
good at; the external URLs the Page has shared are public and can be handled by
ordinary HTTP clients or downstream extractors. Typed extraction is useful here
because the sources are heterogeneous.
...
# In the calling conversation, send external_urls to the downstream extractor
# with whatever prompt and schema match the task.
```
...
send `external_urls` to the downstream extractor, route `meta` into a
